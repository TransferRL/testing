...
class RBM(object):
    ...
    def pcd_k(self, start_v1, start_v2, learning_rate=0.1, k=1, persistent=True):
        "k-step (persistent) contrastive divergence"
        if self.chain_v1 is None and persistent:
            self.chain_v1 = start_v1
        if self.chain_v2 is None and persistent:
            self.chain_v2 = start_v2
        if self.chain_h is None and persistent:
            self.chain_h = self.prop_v1v2_h(self.chain_v1, self.chain_v2)

        mcmc_v1, mcmc_v2, start_h = (self.chain_v1, self.chain_v2, self.chain_h) if persistent else (start_v1, start_v2
            , self.prop_v1v2_h(start_v1, start_v2))
        ...
        update_v1b = self.v1_bias.assign_add(learning_rate * tf.reduce_mean(start_v1 - mcmc_v1, 0))
        update_v2b = self.v1_bias.assign_add(learning_rate * tf.reduce_mean(start_v2 - mcmc_v2, 0))

        update_hb = self.h_bias.assign_add(learning_rate * tf.reduce_mean(start_h - mcmc_h, 0))
    ...
    def reconstruction_error(self, v1, v2):
        ...
        v1_err = tf.stop_gradient(v1 - self.sample_v1_given_v2h(v2, h))
        v1_err = tf.reduce_sum(v1_err * v1_err, [0, 1])
        
        v2_err = tf.stop_gradient(v2 - self.sample_v2_given_v1h(v1, h))
