...
class RBM(object):
    ...
    def __init__(self, name, v1_size, h_size, v2_size, n_data, batch_size, num_epochs=100, learning_rate=0.1, k=1,
                 persistent=False, use_tqdm=True, show_err_plt=True):
    ...
    def sample_v1_given_v2h(self, v2, h):
        ...
        dist = tf.contrib.distributions.Normal(tf.cast(self.prop_v2h_v1(v2, h, v2.shape[0]), tf.float32),
                                               tf.cast(tf.tile(tf.expand_dims(self.v1_var, 0), [v2.shape[0], 1]),
                                                       tf.float32))
        return tf.reduce_sum(dist.sample(1), 0)

    def sample_v2_given_v1h(self, v1, h):
        """ generate sample of v1 from v2 and h"""
        dist = tf.contrib.distributions.Normal(tf.cast(self.prop_v1h_v2(v1, h, v1.shape[0]), tf.float32),
                                               tf.cast(tf.tile(tf.expand_dims(self.v2_var, 0), [v1.shape[0], 1]),
                                                       tf.float32))
        return tf.reduce_sum(dist.sample(1), 0)
    ...
    def pcd_k(self):
        ...
        if self.chain_h is None and self.persistent:
            self.chain_h = self.prop_v1v2_h(self.chain_v1, self.chain_v2, self.batch_size)

        mcmc_v1, mcmc_v2 = (self.chain_v1, self.chain_v2) if self.persistent else (self.v1_input, self.v2_input)

        start_h = self.chain_h if self.persistent else self.prop_v1v2_h(self.v1_input, self.v2_input, self.batch_size)
        mcmc_h = start_h

        for n in range(self.k):
            mcmc_v1, mcmc_h, mcmc_v2 = self.gibbs(mcmc_v1, mcmc_h, mcmc_v2, self.batch_size)
